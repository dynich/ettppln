...
export const createPiket = async (req, res) => {
  try {
    ...
    const {
      ...
      pekerjaanLebih,
      statusApproval,
      buktiPiket = req.file ? `uploads/${req.file.filename}` : null,
    } = req.body;
...
export const updateStatus = async (req, res) => {
  try {
    ...
    if (req.role === "admin1" && piket.admin1Approval === "Menunggu") {
      ...
      req.role === "superadmin" &&
      piket.superadminApproval === "Menunggu"
    ) {
    ...
    if (
      piket.admin1Approval === "Disetujui" &&
      piket.admin2Approval === "Disetujui" &&
      piket.superadminApproval === "Disetujui"
    ) {
      ...
      const jumlahHari = 1;
      if (
        piket.jenisPiket === "Piket Rutin" &&
        piket.jenisHari === "Hari Kerja"
      ) {
        piket.bayarPiketRutin = calculatePiket(
          piket.jenisPiket,
          piket.jenisHari,
          jumlahHari
        );
        ...
        piket.jenisPiket === "Piket Rutin" &&
        (piket.jenisHari === "Hari Libur Bersama" ||
          piket.jenisHari === "Hari Libur Nasional")
      ) {
        piket.bayarPiketRutin = calculatePiket(
          piket.jenisPiket,
          piket.jenisHari,
        ...
        piket.bayarPiketKhusus = calculatePiket(
          piket.jenisPiket,
          piket.jenisHari,
          jumlahHari
        );
      ...
      piket.admin1Approval === "Ditolak" ||
      piket.admin2Approval === "Ditolak" ||
      piket.superadminApproval === "Ditolak"
    ) {
