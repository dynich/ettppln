...
const LemburList = () => {
  const [lemburs, setLemburs] = useState([]);
  const [tanggal, setTanggal] = useState("");
  const [jamMulai, setJamMulai] = useState("");
  const [jamSelesai, setJamSelesai] = useState("");
  const [jumlahJamLembur, setJumlahJamLembur] = useState("");
  const [jenisHari, setJenisHari] = useState("Hari Kerja");
  const [pekerjaanLebih, setPekerjaanLebih] = useState("");
  const [buktiLembur, setBuktiLembur] = useState(null);
  ...
  const submitLembur = (e) => {
    ...
    Modal.confirm({
      ...
      content: "Are you sure you want to submit this lembur?",
      onOk: async () => {
        try {
          ...
          formData.append("jumlahJamLembur", jumlahJamLembur);
  ...
  const data = lemburs.map((lembur, index) => {
    const rowData = {
      ...
      bagian: lembur.user.bagian,
      unitKerja: lembur.user.unitKerja,
      atasan: lembur.user.atasan,
      tanggal2: formatDate(lembur.tanggal),
      tanggal1: formatDateWithoutDay(lembur.tanggal),
      tanggal: lembur.tanggal,
      jamMulai: lembur.jamMulai,
      jamSelesai: lembur.jamSelesai,
      totalJam: lembur.jumlahJamLembur,
  ...
  const generatePDF = async (rowData, users) => {
    ...
    const atasanName = rowData.atasan; // Nama atasan dari data lembur
    let atasanJabatan = "";
    ...
    img.onload = function () {
      ...
      pdf.text("Total Jam Lembur", 20, 113);
      pdf.text(`: ${rowData.totalJam} Jam`, 70, 113);
      pdf.text("Jenis Lembur", 20, 121);
      pdf.text(": Lembur", 70, 121);
      pdf.text("Pekerjaan Kerja Lebih", 20, 129);
      pdf.text(`: ${rowData.pekerjaanLebih}`, 70, 129);
    ...
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        <h3 className="judul_font" style={{ color: "#168FF0" }}>
          Buat Surat Perintah Kerja Lebih mu!
        </h3>
        <p className="text_isi">Silahkan isi dengan sebenar-benarnya!</p>

        <div>
          <form onSubmit={submitLembur}>
            <Row gutter={16}>
              <Col span={12}>
